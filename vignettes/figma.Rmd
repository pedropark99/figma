...
the data of your Figma file as raw bytes. So you need to convert these bytes in a more useful 
format like a JSON object, or a character vector, or as pure text.

```{r}
head(figma::untitled_file$content)
```

To do this conversion, I recommend you to use the `httr::content()` function

```{r}
figma::untitled_file |>
  httr::content(as = "text") |>
  substr(1, 50) # Truncate/limit very long string to first 50 characters
```
